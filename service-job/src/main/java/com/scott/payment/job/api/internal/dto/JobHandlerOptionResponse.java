...
public class JobHandlerOptionResponse {
    ...
    /**
     * {@code handlerName}，用于展示或识别当前商户、渠道、用户、角色、模板或配置对象。
     * <p>
    ...
    /**
     * {@code allowManualTrigger}，用于明确 {@code JobHandlerOptionResponse} 当前业务分支是否成立。
     * <p>
    ...
    /**
     * {@code allowConcurrent}，用于明确 {@code JobHandlerOptionResponse} 当前业务分支是否成立。
     * <p>
     * 单位：无；格式：布尔值或 0/1 开关；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：仅允许平台约定的启停取值；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * </p>

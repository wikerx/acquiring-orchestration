 ...
 * @author : scott
 * @version : v1.0.0
 * @classname : VoidPreparationResultDTO
 * @date : 2026-07-24 00:00
 * @description : Void 本地准备结果 DTO，承载已提交的撤销动作事实、幂等结果和渠道请求身份。
 * @status : create
 */
@Data
public class VoidPreparationResultDTO implements Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * call Channel，用于保存 Void Preparation Result DTO 中与 call渠道 相关的业务属性。
     * <p>
    ...
    /**
     * duplicate，用于保存 Void Preparation Result DTO 中与 duplicate 相关的业务属性。
     * <p>
     * 单位：无；格式：布尔值或 0/1 开关；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：仅允许平台约定的启停取值；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
    ...
    /**
     * idempotency Key，用于保存 Void Preparation Result DTO 中与 idempotency密钥 相关的业务属性。
     * <p>
    ...
    /**
     * command DTO，用于保存 Void Preparation Result DTO 中与 commanddto 相关的业务属性。
     * <p>
    ...
    /**
     * source Order DO，用于保存 Void Preparation Result DTO 中与 来源订单do 相关的业务属性。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
    ...
    /**
     * route Result DTO，用于保存 Void Preparation Result DTO 中与 routeresultdto 相关的业务属性。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
    ...
    /**
     * prepared Channel Request DTO，用于保存 Void Preparation Result DTO 中与 prepared渠道requestdto 相关的业务属性。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
    ...
    /**
     * result DTO，用于保存 Void Preparation Result DTO 中与 resultdto 相关的业务属性。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
     ...
     */
    public static VoidPreparationResultDTO duplicate(PaymentCreateResultDTO resultDTO) {

...
public class RefundApprovalResultDTO implements Serializable {
    ...
    /**
     * 审批ID，用于定位 {@code RefundApprovalResultDTO} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
    ...
    /**
     * {@code approvalOperatorId}，用于定位 {@code RefundApprovalResultDTO} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：无；格式：业务编号字符串；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：长度、唯一性和可空性由接口校验或数据库唯一约束限制；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * </p>
    ...
    /**
     * 审批时间字段，保存 {@code RefundApprovalResultDTO} 当前处理所需的业务取值。
     * <p>
    ...
    /**
     * 审批原因字段，保存 {@code RefundApprovalResultDTO} 当前处理所需的业务取值。
     * <p>
    ...
    /**
     * {@code executionEventId}，用于定位 {@code RefundApprovalResultDTO} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：无；格式：业务编号字符串；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：长度、唯一性和可空性由接口校验或数据库唯一约束限制；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * </p>

    ...
    private void createPhysicalTables() {
        ...
        for (String suffix : List.of("202603", "202604")) {
            direct.execute("CREATE TABLE transaction_order_" + suffix + " ("
                    ...
                    + "root_transaction_id VARCHAR(64), latest_transaction_id VARCHAR(64), "
                    + "merchant_id VARCHAR(64), merchant_order_no VARCHAR(128), merchant_order_id VARCHAR(128), "
                    + "merchant_transaction_id VARCHAR(128), source_transaction_id VARCHAR(64), "
                    + "payment_method VARCHAR(32), payment_brand VARCHAR(32), transaction_type VARCHAR(32), "
                    + "transaction_status VARCHAR(32), process_stage VARCHAR(64), pending_reason_code VARCHAR(64), "
                    + "fail_reason_code VARCHAR(64), fail_reason_message VARCHAR(512), "
                    + "merchant_visible_message VARCHAR(512), payer_visible_message VARCHAR(512), "
                    + "label_currency CHAR(3), label_amount DECIMAL(20,6), "
                    + "transaction_currency CHAR(3), transaction_amount DECIMAL(20,6), "
                    + "channel_request_currency CHAR(3), channel_request_amount DECIMAL(20,6), "
                    + "settlement_currency CHAR(3), settlement_amount DECIMAL(24,8), "
                    + "settlement_rate DECIMAL(24,12), settlement_date DATE, "
                    + "currency_exponent INT, dcc_enabled INT, edc_enabled INT, "
                    + "transaction_rate DECIMAL(24,12), rate_source VARCHAR(64), rate_time TIMESTAMP(3), "
                    + "authorized_amount DECIMAL(20,6), authorized_cancel_amount DECIMAL(20,6), "
                    + "captured_amount DECIMAL(20,6), refunded_amount DECIMAL(20,6), "
                    + "chargeback_amount DECIMAL(20,6), available_capture_amount DECIMAL(20,6), "
                    + "available_refund_amount DECIMAL(20,6), settlement_status VARCHAR(32), "
                    + "reconciliation_status VARCHAR(32), accounting_status VARCHAR(32), "
                    + "channel_match_status VARCHAR(32), channel_match_result VARCHAR(64), "
                    + "channel_match_count INT, last_channel_match_request_id VARCHAR(64), "
                    + "last_channel_match_time TIMESTAMP(3), next_channel_match_time TIMESTAMP(3), "
                    + "channel_match_fail_reason VARCHAR(512), settlement_batch_no VARCHAR(19), "
                    + "settlement_transaction_id VARCHAR(64), settlement_transaction_date_time TIMESTAMP(3), "
                    + "reconciliation_batch_no VARCHAR(64), channel_id BIGINT, channel_code VARCHAR(32), "
                    + "channel_mid_config_id BIGINT, channel_merchant_id VARCHAR(128), channel_order_no VARCHAR(128), "
                    + "internal_risk_decision VARCHAR(32), internal_risk_record_no VARCHAR(64), "
                    + "merchant_website VARCHAR(512), callback_url VARCHAR(512), redirect_url VARCHAR(512), "
                    + "language VARCHAR(20), transaction_date_time TIMESTAMP(3) NOT NULL, "
                    + "transaction_utc_time TIMESTAMP(3), transaction_time_zone VARCHAR(64), "
                    + "transaction_timezone_offset VARCHAR(6), last_status_time TIMESTAMP(3), "
            ...
            direct.execute("CREATE TABLE transaction_operation_" + suffix + " ("
                    + "id BIGINT AUTO_INCREMENT NOT NULL, operation_id VARCHAR(64) NOT NULL, "
                    + "transaction_id VARCHAR(64) NOT NULL, source_transaction_id VARCHAR(64), "
                    + "source_operation_id VARCHAR(64), merchant_id VARCHAR(64), merchant_order_no VARCHAR(128), "
                    + "merchant_order_id VARCHAR(128), merchant_operation_no VARCHAR(128), "
                    + "request_source VARCHAR(32), refund_scope VARCHAR(32), request_reason VARCHAR(512), "
                    + "description VARCHAR(128), applicant_type VARCHAR(32), applicant_id VARCHAR(128), "
                    + "applicant_name VARCHAR(128), execution_mode VARCHAR(32), operation_sequence INT, "
                    + "transaction_type VARCHAR(32), transaction_status VARCHAR(32), process_stage VARCHAR(64), "
                    + "pending_reason_code VARCHAR(64), fail_reason_code VARCHAR(64), fail_reason_message VARCHAR(512), "
                    + "label_currency CHAR(3), label_amount DECIMAL(20,6), "
                    + "transaction_currency CHAR(3), transaction_amount DECIMAL(20,6), "
                    + "approved_currency CHAR(3), approved_amount DECIMAL(20,6), "
                    + "channel_request_currency CHAR(3), channel_request_amount DECIMAL(20,6), "
                    + "settlement_currency CHAR(3), settlement_amount DECIMAL(24,8), "
                    ...
                    + "settlement_status VARCHAR(32), reconciliation_status VARCHAR(32), accounting_status VARCHAR(32), "
                    + "channel_match_status VARCHAR(32), channel_match_result VARCHAR(64), channel_match_count INT, "
                    + "last_channel_match_request_id VARCHAR(64), last_channel_match_time TIMESTAMP(3), "
                    + "next_channel_match_time TIMESTAMP(3), channel_match_fail_reason VARCHAR(512), "
                    + "transaction_date_time TIMESTAMP(3) NOT NULL, transaction_utc_time TIMESTAMP(3), "
                    + "transaction_time_zone VARCHAR(64), operation_time TIMESTAMP(3), complete_time TIMESTAMP(3), "
                    + "version INT NOT NULL, deleted INT NOT NULL, create_time TIMESTAMP(3), update_time TIMESTAMP(3))");

...
public class SysRolePermissionAuthDTO implements Serializable {
    ...
    /**
     * 角色ID，用于定位 {@code SysRolePermissionAuthDTO} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
    ...
    /**
     * 权限集合，承载 {@code SysRolePermissionAuthDTO} 当前请求或响应中的多值数据。
     * <p>
    ...
    /**
     * {@code checkedPermissionIds}集合，承载 {@code SysRolePermissionAuthDTO} 当前请求或响应中的多值数据。
     * <p>
     * 单位：无；格式：集合或键值映射；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：元素类型和数量由所属请求、响应或聚合模型约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：集合元素必须沿用所属模型的主键、币种、状态和数据范围口径。

...
public class IsoCurrencyExportRow {
    ...
    /**
     * numeric Code，用于在系统、渠道、字典或配置中稳定引用当前业务取值。
     * <p>
     * 单位：无；格式：枚举编码或受控字符串；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值必须来自对应枚举、字典或渠道协议；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。
    ...
    /**
     * fraction Digits，用于保存 ISO Currency Export Row 中与 fractiondigits 相关的业务属性。
     * <p>
    ...
    /**
     * created At，用于保存 ISO Currency Export Row 中与 createdat 相关的业务属性。
     * <p>

...
public class SysDictDataExportRow {
    ...
    /**
     * {@code dictType}，用于区分 {@code SysDictDataExportRow} 记录的处理类别、配置维度或外部协议枚举。
     * <p>
    ...
    /**
     * {@code dictLabel}字段，保存 {@code SysDictDataExportRow} 当前处理所需的业务取值。
     * <p>
    ...
    /**
     * {@code dictValue}字段，保存 {@code SysDictDataExportRow} 当前处理所需的业务取值。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>
    ...
    /**
     * 默认标识字段，保存 {@code SysDictDataExportRow} 当前处理所需的业务取值。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>

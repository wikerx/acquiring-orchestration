 ...
 * @description : 汇率汇率usage快照导出行模型，位于 运营后台服务，定义 Excel 列及运营可见值，不承载数据库写入规则。
 * @status : create
 */
@Data
public class ExchangeRateUsageSnapshotExportRow {
    ...
    /**
     * {@code usageScene}字段，保存 {@code ExchangeRateUsageSnapshotExportRow} 当前处理所需的业务取值。
     * <p>
    ...
    /**
     * {@code businessNo}字段，保存 {@code ExchangeRateUsageSnapshotExportRow} 当前处理所需的业务取值。
     * <p>
    ...
    /**
     * {@code usedRate}字段，保存 {@code ExchangeRateUsageSnapshotExportRow} 当前处理所需的业务取值。
     * <p>
     * 单位：比例值；格式：decimal，按费率或汇率精度保存；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围由费率、汇率或预警配置定义；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>
    ...
    /**
     * 原始汇率ID，用于定位 {@code ExchangeRateUsageSnapshotExportRow} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：比例值；格式：decimal，按费率或汇率精度保存；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围由费率、汇率或预警配置定义；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>
    ...
    /**
     * 规则ID，用于定位 {@code ExchangeRateUsageSnapshotExportRow} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：无；格式：业务编号字符串；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：长度、唯一性和可空性由接口校验或数据库唯一约束限制；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>
    ...
    /**
     * {@code businessRateId}，用于定位 {@code ExchangeRateUsageSnapshotExportRow} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：比例值；格式：decimal，按费率或汇率精度保存；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围由费率、汇率或预警配置定义；数据来源：当前业务流程上游模型、配置项或数据库查询结果。
     * </p>
    ...
    /**
     * {@code appliedTime}字段，保存 {@code ExchangeRateUsageSnapshotExportRow} 当前处理所需的业务取值。
     * <p>

...
public final class MccRequests {
    ...
    public static class MccRiskPolicyQueryRequest {
        ...
        /**
         * 完成 safe Page No 的本地校验、字段转换或结果组装，供当前调用链继续使用。
         * <p>
         * 层级边界：运营后台服务层；输入来源、输出结构和异常语义由 MccRiskPolicyQueryRequest 的方法签名及调用链约束。
        ...
        /**
         * 完成 safe Page Size 的本地校验、字段转换或结果组装，供当前调用链继续使用。
         * <p>
         * 层级边界：运营后台服务层；输入来源、输出结构和异常语义由 MccRiskPolicyQueryRequest 的方法签名及调用链约束。
         * 状态变更、事务提交、MQ 投递、远程调用和敏感数据处理以当前方法实现为准，调用方需沿用既有幂等与脱敏约束。
         * </p>

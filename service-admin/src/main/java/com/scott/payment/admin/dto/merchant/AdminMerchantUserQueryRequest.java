 ...
 * @description : Admin Merchant User Query Request 传输模型，位于 运营后台服务，定义接口或跨服务调用字段，承载标识、状态、金额、配置或响应摘要，不直接执行业务逻辑。
 * @status : create
 */
public class AdminMerchantUserQueryRequest extends PageRequest {
    ...
    /**
     * mobile，用于保存 Admin Merchant User Query Request 中与 mobile 相关的业务属性。
     * <p>
    ...
    /**
     * created Start Time，用于保存 Admin Merchant User Query Request 中与 createdstarttime 相关的业务属性。
     * <p>
    ...
    /**
     * created End Time，用于保存 Admin Merchant User Query Request 中与 createdendtime 相关的业务属性。
     * <p>
     * 单位：系统业务时区时间；格式：ISO 日期或日期时间；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：时间范围由业务流程或查询条件限定；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。

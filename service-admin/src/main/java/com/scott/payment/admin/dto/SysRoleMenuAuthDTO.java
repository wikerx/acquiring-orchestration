...
public class SysRoleMenuAuthDTO implements Serializable {
    ...
    /**
     * role ID，用于定位 Sys Role Menu Auth DTO 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
    ...
    /**
     * menus，用于保存 Sys Role Menu Auth DTO 中与 菜单 相关的业务属性。
     * <p>
    ...
    /**
     * checked Menu Ids，用于保存 Sys Role Menu Auth DTO 中与 checked菜单ID 相关的业务属性。
     * <p>
     * 单位：无；格式：字符串、对象引用或集合结构；是否允许为空由接口校验、数据库约束或调用契约决定；非敏感字段。
     * 取值范围：取值范围受数据库字段长度、Bean Validation、接口协议或配置枚举约束；数据来源：上游接口请求、内部服务调用或远程服务响应。
     * 字段关系：与同记录的主键、业务编号、状态和审计时间一起用于查询、展示或排障。

 ...
 * @description : 资金账户、充值和扣减审批实现；实时聚合在途资金，并以账户锁串行化余额变动。
 * @status : create
 */
@Service
public class AdminFundAccountServiceImpl implements AdminFundAccountService {
    ...
    public PageResult<FundLedgerResponse> pageLedgers(Long accountId, FundDetailQuery request) {
        ...
        Map<String, MerchantFundDeductionDO> deductions = deductionMap(page.getRecords());
        return PageResult.of(page.getTotal(), page.getCurrent(), page.getSize(),
                page.getRecords().stream().map(row -> toLedger(row, account, null,
                        recharges.get(row.getBusinessNo()), deductions.get(row.getBusinessNo()))).toList());
    }
    ...
    public PageResult<FundLedgerResponse> pageAllLedgers(FundDetailQuery request) {
        ...
        List<FundLedgerResponse> records = page.getRecords().stream().map(row -> {
            ...
            return toLedger(row, account, names.get(row.getMerchantId()),
                    recharges.get(row.getBusinessNo()), deductions.get(row.getBusinessNo()));
        }).toList();
        return PageResult.of(page.getTotal(), page.getCurrent(), page.getSize(), records);
    }
    ...
    @Override
    @DS(DataSourceName.SLAVE)
    public PageResult<FundDeductionResponse> pageDeductions(FundDeductionQuery request) {
        FundDeductionQuery query = request == null ? new FundDeductionQuery() : request;
        LambdaQueryWrapper<MerchantFundDeductionDO> wrapper = Wrappers.<MerchantFundDeductionDO>lambdaQuery()
                .eq(MerchantFundDeductionDO::getDeleted, 0L)
                .eq(StringUtils.hasText(query.getMerchantId()), MerchantFundDeductionDO::getMerchantId,
                        trim(query.getMerchantId()))
                .eq(StringUtils.hasText(query.getDeductionCategory()),
                        MerchantFundDeductionDO::getDeductionCategory, upper(query.getDeductionCategory()))
                .eq(StringUtils.hasText(query.getDeductionStatus()), MerchantFundDeductionDO::getDeductionStatus,
                        upper(query.getDeductionStatus()))
                .orderByDesc(MerchantFundDeductionDO::getCreateTime)
                .orderByDesc(MerchantFundDeductionDO::getId);
        if (StringUtils.hasText(query.getKeyword())) {
            String keyword = query.getKeyword().trim();
            wrapper.and(value -> value.like(MerchantFundDeductionDO::getDeductionNo, keyword)
                    .or().like(MerchantFundDeductionDO::getRequestId, keyword)
                    .or().like(MerchantFundDeductionDO::getMerchantId, keyword));
        }
        Page<MerchantFundDeductionDO> page = deductionMapper.selectPage(
                new Page<>(query.safePageNo(), query.safePageSize()), wrapper);
        Map<Long, MerchantFundAccountDO> accounts = accountMap(page.getRecords().stream()
                .map(MerchantFundDeductionDO::getAccountId).collect(Collectors.toSet()));
        Map<String, String> names = merchantNames(page.getRecords().stream()
                .map(MerchantFundDeductionDO::getMerchantId).collect(Collectors.toSet()));
        return PageResult.of(page.getTotal(), page.getCurrent(), page.getSize(), page.getRecords().stream()
                .map(row -> toDeduction(row, accounts.get(row.getAccountId()), names.get(row.getMerchantId())))
                .toList());
    }
    ...
    public FundDeductionResponse getDeduction(Long id) {
        MerchantFundDeductionDO deduction = deductionMapper.selectOne(
                Wrappers.<MerchantFundDeductionDO>lambdaQuery()
                        .eq(MerchantFundDeductionDO::getId, id)
                        .eq(MerchantFundDeductionDO::getDeleted, 0L)
                        .last("LIMIT 1"));
        if (deduction == null) {
            throw new ServiceException(ApiResultEnum.NOT_FOUND.getCode(), "扣减申请不存在");
        }
        return deductionResponse(deduction);
    }
    ...
    public FundDeductionResponse createDeduction(FundDeductionCreateRequest request,
                                                 ...
                                                 String loginAccount) {
        MerchantFundAccountDO account = requireAccount(request.getAccountId());
        if (CLOSED.equals(normalizeManualStatus(account.getAccountStatus()))) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "已关闭的资金账户不允许扣减余额");
        }
        validateDeductionAmount(request.getAmount());
        String category = validateDeductionCategory(request.getDeductionCategory());
        String requestId = request.getRequestId().trim();
        MerchantFundDeductionDO existing = findDeductionByRequestId(requestId);
        if (existing != null) {
            return resolveIdempotentDeduction(existing, account, request.getAmount(), category);
        }
        LocalDateTime now = LocalDateTime.now();
        MerchantFundDeductionDO deduction = new MerchantFundDeductionDO();
        deduction.setDeductionNo(generateCode("FD"));
        deduction.setAccountId(account.getId());
        deduction.setMerchantId(account.getMerchantId());
        deduction.setCurrency(account.getSettlementCurrency());
        deduction.setAmount(request.getAmount());
        deduction.setDeductionCategory(category);
        deduction.setDeductionStatus(PENDING_AUDIT);
        deduction.setReason(request.getReason().trim());
        deduction.setSubmitById(operatorId);
        deduction.setSubmitByName(operatorName);
        deduction.setSubmitLoginAccount(loginAccount);
        deduction.setSubmitTime(now);
        deduction.setRequestId(requestId);
        deduction.setCreateTime(now);
        deduction.setUpdateTime(now);
        deduction.setDeleted(0L);
        try {
            deductionMapper.insert(deduction);
        } catch (DuplicateKeyException exception) {
            MerchantFundDeductionDO concurrent = deductionMapper.selectByRequestIdForUpdate(requestId);
            if (concurrent == null) {
                throw exception;
            }
            return resolveIdempotentDeduction(concurrent, account, request.getAmount(), category);
        }
        return toDeduction(deduction, account, merchantName(account.getMerchantId()));
    }
    ...
    @DS(DataSourceName.MASTER)
    @Transactional(rollbackFor = Exception.class)
    public FundDeductionResponse auditDeduction(Long id,
                                                String comment,
                                                Long operatorId,
                                                String operatorName,
                                                String loginAccount) {
        MerchantFundDeductionDO deduction = requireLockedDeduction(id);
        requireDeductionStatus(deduction, PENDING_AUDIT);
        if (!isBuiltinAdmin(loginAccount) && sameOperator(deduction.getSubmitById(), operatorId)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "提交人不能审核自己提交的扣减申请");
        }
        LocalDateTime now = LocalDateTime.now();
        deduction.setAuditById(operatorId);
        deduction.setAuditByName(operatorName);
        deduction.setAuditComment(trimToNull(comment));
        deduction.setAuditTime(now);
        deduction.setDeductionStatus(PENDING_RECHECK);
        deduction.setUpdateTime(now);
    ...
    @DS(DataSourceName.MASTER)
    @Transactional(rollbackFor = Exception.class)
    public FundDeductionResponse recheckDeduction(Long id,
                                                  String comment,
                                                  Long operatorId,
                                                  String operatorName,
                                                  String loginAccount) {
        MerchantFundDeductionDO deduction = requireLockedDeduction(id);
        requireDeductionStatus(deduction, PENDING_RECHECK);
        if (!isBuiltinAdmin(loginAccount) && sameOperator(deduction.getSubmitById(), operatorId)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "提交人不能复核自己提交的扣减申请");
        }
        if (!isBuiltinAdmin(loginAccount) && sameOperator(deduction.getAuditById(), operatorId)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "普通账号的提交、审核和复核必须由三个人完成");
        }
        MerchantFundAccountDO account = accountMapper.selectByIdForUpdate(deduction.getAccountId());
        if (account == null || !account.getMerchantId().equals(deduction.getMerchantId())) {
            throw new ServiceException(ApiResultEnum.NOT_FOUND.getCode(), "扣减资金账户不存在");
        }
        if (CLOSED.equals(normalizeManualStatus(account.getAccountStatus()))) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "已关闭的资金账户不允许扣减余额");
        }
        LocalDateTime now = LocalDateTime.now();
        BigDecimal before = account.getAvailableBalance();
        BigDecimal after = before.subtract(deduction.getAmount());
        account.setAvailableBalance(after);
        account.setAccountVersion(account.getAccountVersion() + 1);
        account.setAccountStatus(normalizeManualStatus(account.getAccountStatus()));
        account.setUpdateBy(operatorName);
        ...
        MerchantFundLedgerDO ledger = buildDeductionLedger(deduction, account, before, after,
                comment, operatorId, operatorName, now);
        ledgerMapper.insert(ledger);
        deduction.setRecheckById(operatorId);
        deduction.setRecheckByName(operatorName);
        deduction.setRecheckComment(trimToNull(comment));
        deduction.setRecheckTime(now);
        deduction.setDeductionStatus(POSTED);
        deduction.setLedgerNo(ledger.getLedgerNo());
        deduction.setPostedTime(now);
        deduction.setUpdateTime(now);
        deductionMapper.updateById(deduction);
        return toDeduction(deduction, account, merchantName(account.getMerchantId()));
    }
    ...
    @DS(DataSourceName.MASTER)
    @Transactional(rollbackFor = Exception.class)
    public FundDeductionResponse rejectDeduction(Long id,
                                                 String comment,
                                                 Long operatorId,
                                                 String operatorName,
                                                 String loginAccount) {
        MerchantFundDeductionDO deduction = requireLockedDeduction(id);
        if (!Set.of(PENDING_AUDIT, PENDING_RECHECK).contains(deduction.getDeductionStatus())) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "当前扣减状态不允许驳回");
        }
        if (sameOperator(deduction.getSubmitById(), operatorId)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "提交人不能驳回自己提交的扣减申请");
        }
        LocalDateTime now = LocalDateTime.now();
        deduction.setDeductionStatus(REJECTED);
        deduction.setRejectById(operatorId);
        deduction.setRejectByName(operatorName);
        deduction.setRejectComment(comment.trim());
        deduction.setRejectTime(now);
        deduction.setUpdateTime(now);
        deductionMapper.updateById(deduction);
        return deductionResponse(deduction);
    }
    ...
    /**
     * 批量读取当前页扣减流水对应的审批快照，避免详情展示产生逐行查询。
     *
     * @param ledgers 当前页不可变余额流水
     * @return 以扣减单号为键的有效扣减申请；没有扣减流水时返回空映射
     */
    private Map<String, MerchantFundDeductionDO> deductionMap(List<MerchantFundLedgerDO> ledgers) {
        Set<String> deductionNos = ledgers.stream()
                .filter(row -> "BALANCE_DEDUCTION".equals(row.getBusinessType()))
                .map(MerchantFundLedgerDO::getBusinessNo)
                .filter(StringUtils::hasText)
                .collect(Collectors.toSet());
        if (deductionNos.isEmpty()) {
            return Map.of();
        }
        List<MerchantFundDeductionDO> deductions = deductionMapper.selectList(
                Wrappers.<MerchantFundDeductionDO>lambdaQuery()
                        .in(MerchantFundDeductionDO::getDeductionNo, deductionNos)
                        .eq(MerchantFundDeductionDO::getDeleted, 0L));
        if (deductions == null || deductions.isEmpty()) {
            return Map.of();
        }
        return deductions.stream().collect(Collectors.toMap(MerchantFundDeductionDO::getDeductionNo,
                Function.identity(), (left, right) -> left));
    }
     ...
     * @param deduction 扣减审批快照，非扣减流水时为空
     * @return 管理端余额流水响应
     */
    private FundLedgerResponse toLedger(MerchantFundLedgerDO row,
                                        ...
                                        MerchantFundDeductionDO deduction) {
        ...
        response.setMerchantName(merchantName);
        response.setBalanceType("AVAILABLE");
        ...
        if (deduction != null) {
            response.setDeductionDetail(toDeduction(deduction, account, merchantName));
        }
        return response;
    }
    ...
    /**
     * 在账户行锁保护下构造扣减入账流水；金额保持正数，以借方方向表达余额减少。
     *
     * @param deduction 已完成审核、当前正在复核的扣减申请
     * @param account 已持有排他锁的资金账户
     * @param before 扣减前可用余额，单位为账户结算币种
     * @param after 扣减后可用余额，允许为负
     * @param recheckComment 复核意见，允许为空
     * @param recheckById 复核人账号主键
     * @param recheckByName 复核人名称快照
     * @param now 本次原子入账使用的统一系统时间
     * @return 待插入的不可变扣减余额流水
     */
    private MerchantFundLedgerDO buildDeductionLedger(MerchantFundDeductionDO deduction,
                                                       MerchantFundAccountDO account,
                                                       BigDecimal before,
                                                       BigDecimal after,
                                                       String recheckComment,
        ...
        MerchantFundLedgerDO ledger = new MerchantFundLedgerDO();
        ledger.setLedgerNo(generateCode("FL"));
        ledger.setLedgerGroupNo(deduction.getDeductionNo());
        ledger.setAccountId(account.getId());
        ledger.setMerchantId(account.getMerchantId());
        ledger.setBusinessType("BALANCE_DEDUCTION");
        ledger.setSummary("账户扣减（" + deductionCategoryName(deduction.getDeductionCategory()) + "）");
        ledger.setBusinessNo(deduction.getDeductionNo());
        ledger.setCurrency(account.getSettlementCurrency());
        ledger.setDirection("DEBIT");
        ledger.setAmount(deduction.getAmount());
        ledger.setBalanceBefore(before);
        ledger.setBalanceAfter(after);
        ledger.setAccountSequence(ledgerMapper.selectMaxAccountSequence(account.getId()) + 1);
        ledger.setOperationMode("MANUAL");
        ledger.setOperatorId(deduction.getSubmitById());
        ledger.setOperatorName(deduction.getSubmitByName());
        ledger.setReviewerId(recheckById);
        ledger.setReviewerName(recheckByName);
        ledger.setOperationReason(deduction.getReason());
        ledger.setReviewComment(joinReviewComments(deduction.getAuditComment(), recheckComment));
        ledger.setBusinessTime(deduction.getSubmitTime());
        ledger.setSubmitTime(deduction.getSubmitTime());
        ledger.setReviewTime(now);
        ledger.setPostedTime(now);
        ledger.setRequestId(deduction.getRequestId());
        ledger.setIdempotencyKey("FUND_DEDUCTION:" + deduction.getDeductionNo());
        ledger.setCreateTime(now);
        return ledger;
    }
    ...
    /** 按申请关联账户和商户名称构造扣减响应。 */
    private FundDeductionResponse deductionResponse(MerchantFundDeductionDO deduction) {
        MerchantFundAccountDO account = requireAccount(deduction.getAccountId());
        return toDeduction(deduction, account, merchantName(deduction.getMerchantId()));
    }

    /** 将扣减审批持久化快照转换为管理端响应。 */
    private FundDeductionResponse toDeduction(MerchantFundDeductionDO row,
                                              MerchantFundAccountDO account,
                                              String merchantName) {
        FundDeductionResponse response = new FundDeductionResponse();
        response.setId(row.getId());
        response.setDeductionNo(row.getDeductionNo());
        response.setAccountId(row.getAccountId());
        response.setAccountNo(account == null ? null : account.getAccountNo());
        response.setMerchantId(row.getMerchantId());
        response.setMerchantName(merchantName);
        response.setCurrency(row.getCurrency());
        response.setAmount(row.getAmount());
        response.setDeductionCategory(row.getDeductionCategory());
        response.setDeductionStatus(row.getDeductionStatus());
    ...
    /**
     * 解析相同请求号的扣减申请；账户、类型和金额完全一致才视为幂等重试。
     */
    private FundDeductionResponse resolveIdempotentDeduction(MerchantFundDeductionDO existing,
                                                              MerchantFundAccountDO account,
                                                              BigDecimal amount,
                                                              String category) {
        if (!existing.getAccountId().equals(account.getId())
                || existing.getAmount().compareTo(amount) != 0
                || !existing.getDeductionCategory().equals(category)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "请求号已被其他扣减申请使用");
        }
        return deductionResponse(existing);
    }

    /** 按客户端请求号读取未删除扣减申请，唯一键提供最终并发幂等保障。 */
    private MerchantFundDeductionDO findDeductionByRequestId(String requestId) {
        return deductionMapper.selectOne(Wrappers.<MerchantFundDeductionDO>lambdaQuery()
                .eq(MerchantFundDeductionDO::getRequestId, requestId)
                .eq(MerchantFundDeductionDO::getDeleted, 0L)
                .last("LIMIT 1"));
    }

    /** 通过主库排他锁读取扣减申请，串行化审批状态变更和最终入账。 */
    private MerchantFundDeductionDO requireLockedDeduction(Long id) {
        MerchantFundDeductionDO deduction = deductionMapper.selectByIdForUpdate(id);
        if (deduction == null) {
            throw new ServiceException(ApiResultEnum.NOT_FOUND.getCode(), "扣减申请不存在");
        }
        return deduction;
    ...
    /** 校验扣减申请仍处于预期状态，拒绝重复审批或并发覆盖。 */
    private void requireDeductionStatus(MerchantFundDeductionDO deduction, String expectedStatus) {
        if (!expectedStatus.equals(deduction.getDeductionStatus())) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "扣减申请状态已变化，请刷新后重试");
        }
    }

    /** 校验服务层扣减金额边界，覆盖绕过 Controller 校验的内部调用。 */
    private void validateDeductionAmount(BigDecimal amount) {
        if (amount == null || amount.signum() <= 0 || amount.compareTo(new BigDecimal("100000000")) > 0) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "扣减金额必须大于 0 且不超过 100,000,000");
        }
    }

    /** 校验并规范化扣减类型。 */
    private String validateDeductionCategory(String value) {
        String category = upper(value);
        if (!Set.of("ACCOUNT_CORRECTION", "EXTRA_FEE", "PENALTY", "OTHER").contains(category)) {
            throw new ServiceException(ApiResultEnum.PARAM_INVALID.getCode(), "扣减类型不合法");
        }
        return category;
    }

    /** 返回用于不可变流水摘要的稳定中文扣减类型名称。 */
    private String deductionCategoryName(String category) {
        return switch (category) {
            case "ACCOUNT_CORRECTION" -> "账务更正";
            case "EXTRA_FEE" -> "额外费用";
            case "PENALTY" -> "罚金";
            default -> "其他";
        };
    ...
    /**
     * 兼容历史余额类型查询参数；当前不可变流水只记录可用余额变动。
     *
     * @param balanceType 历史接口传入的余额类型
     * @return 指定了 AVAILABLE 以外类型时返回 true
     */

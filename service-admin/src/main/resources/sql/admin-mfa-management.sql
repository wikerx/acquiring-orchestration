-- 管理系统 Multi-Factor Authentication (MFA) 落地脚本
...
FROM sys_menu menu
JOIN (
    SELECT 'system_user' menu_code, 'sys:user:mfa:view' permission_code, '查看用户 MFA 状态' permission_name, 'BUTTON' permission_type, 'POST' resource_method, '/admin/system/users/search' resource_path, '查看用户列表中的 MFA 策略、状态和时间信息' description
    UNION ALL SELECT 'system_user', 'sys:user:mfa:require', '强制启用用户 MFA', 'BUTTON', 'POST', '/admin/system/users/mfa/require', '将用户 MFA 策略调整为强制启用'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:exempt', '配置用户 MFA 豁免', 'BUTTON', 'POST', '/admin/system/users/mfa/exempt', '明确配置指定用户无需 MFA 验证'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:disable', '停用用户 MFA', 'BUTTON', 'POST', '/admin/system/users/mfa/disable', '将用户 MFA 恢复为未启用'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:reset', '重置用户 MFA', 'BUTTON', 'POST', '/admin/system/users/mfa/reset', '废弃旧 MFA 密钥并要求重新绑定'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:unlock', '解锁用户 MFA', 'BUTTON', 'POST', '/admin/system/users/mfa/unlock', '解除连续失败导致的 MFA 临时锁定'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:resend', '重发 MFA 绑定邮件', 'BUTTON', 'POST', '/admin/system/users/mfa/resend-bind-mail', '重新发送绑定引导邮件'
    UNION ALL SELECT 'system_user', 'sys:user:mfa:log', '查看用户 MFA 日志', 'BUTTON', 'POST', '/admin/system/users/mfa/logs/search', '查询用户 MFA 相关安全审计日志'
...
FROM (
    SELECT 'ADMIN_MFA_BIND_NOTICE' template_code,
           '管理系统 MFA 绑定通知' template_name,
           '【Vexra Admin】请绑定多因素认证（MFA）' subject_template,
           '<div style="margin:0;padding:36px;background:#F3F7FF;font-family:Arial,''Microsoft YaHei'',sans-serif;color:#0F172A;"><div style="max-width:640px;margin:0 auto;background:#FFFFFF;border:1px solid #DBEAFE;border-radius:8px;overflow:hidden;"><div style="padding:26px 30px;background:#2563EB;color:#FFFFFF;"><div style="font-size:12px;letter-spacing:0;text-transform:uppercase;color:#DBEAFE;">Vexra Admin Security</div><div style="margin-top:8px;font-size:24px;font-weight:700;">多因素认证（MFA）绑定</div></div><div style="padding:30px;line-height:1.8;font-size:14px;"><p style="margin:0 0 14px;">您好，${loginAccount}：</p><p style="margin:0 0 14px;">您的管理系统账号已开启多因素认证（MFA）。请打开管理系统登录页，完成账号密码校验后按页面提示扫码绑定。</p><div style="margin:22px 0;padding:18px 20px;background:#F3F7FF;border:1px solid #DBEAFE;border-radius:8px;"><p style="margin:0 0 8px;">登录账号：<strong>${loginAccount}</strong></p><p style="margin:0 0 8px;">登录地址：<a href="${bindUrl}" style="color:#2563EB;text-decoration:none;">${bindUrl}</a></p><p style="margin:0;">操作原因：${reason}</p></div><p style="margin:0 0 10px;">请使用支持 TOTP 的验证器应用。邮件不会包含 MFA 密钥、二维码或 MFA 验证码。</p><p style="margin:0;color:#9a3412;">如非本人或授权管理员发起，请立即联系系统管理员。</p></div><div style="padding:16px 30px;background:#F3F7FF;color:#64748B;font-size:12px;">Vexra Admin 安全通知，请勿转发。</div></div></div>' content_template,
           '{"loginAccount":"admin@example.com","bindUrl":"https://admin.example.com/login","reason":"安全策略启用"}' variable_schema,
           '系统内置模板：管理系统 MFA 绑定通知' remark
    UNION ALL SELECT 'ADMIN_MFA_ENABLED_NOTICE',
           '管理系统 MFA 启用通知',
           '【Vexra Admin】多因素认证（MFA）已开启',
           '<div style="margin:0;padding:36px;background:#F3F7FF;font-family:Arial,''Microsoft YaHei'',sans-serif;color:#0F172A;"><div style="max-width:640px;margin:0 auto;background:#FFFFFF;border:1px solid #DBEAFE;border-radius:8px;overflow:hidden;"><div style="padding:26px 30px;background:#2563EB;color:#FFFFFF;"><div style="font-size:12px;letter-spacing:0;text-transform:uppercase;color:#DBEAFE;">Vexra Admin Security</div><div style="margin-top:8px;font-size:24px;font-weight:700;">多因素认证（MFA）已开启</div></div><div style="padding:30px;line-height:1.8;font-size:14px;"><p style="margin:0 0 14px;">您好，${loginAccount}：</p><p style="margin:0 0 14px;">管理员已为您的管理系统账号开启多因素认证（MFA）。下次登录时，您需要先完成绑定，再进入系统。</p><div style="margin:22px 0;padding:18px 20px;background:#F3F7FF;border:1px solid #DBEAFE;border-radius:8px;"><p style="margin:0 0 8px;">登录账号：<strong>${loginAccount}</strong></p><p style="margin:0;">操作原因：${reason}</p></div><p style="margin:0;">请前往 <a href="${bindUrl}" style="color:#2563EB;text-decoration:none;">管理系统登录页</a> 完成绑定。</p></div><div style="padding:16px 30px;background:#F3F7FF;color:#64748B;font-size:12px;">Vexra Admin 安全通知，请勿转发。</div></div></div>',
           '{"loginAccount":"admin@example.com","bindUrl":"https://admin.example.com/login","reason":"安全策略启用"}',
           '系统内置模板：管理系统 MFA 启用通知'
    UNION ALL SELECT 'ADMIN_MFA_RESET_NOTICE',
           '管理系统 MFA 重置通知',
           '【Vexra Admin】多因素认证（MFA）已重置',
           '<div style="margin:0;padding:36px;background:#F3F7FF;font-family:Arial,''Microsoft YaHei'',sans-serif;color:#0F172A;"><div style="max-width:640px;margin:0 auto;background:#FFFFFF;border:1px solid #DBEAFE;border-radius:8px;overflow:hidden;"><div style="padding:26px 30px;background:#2563EB;color:#FFFFFF;"><div style="font-size:12px;letter-spacing:0;text-transform:uppercase;color:#DBEAFE;">Vexra Admin Security</div><div style="margin-top:8px;font-size:24px;font-weight:700;">多因素认证（MFA）已重置</div></div><div style="padding:30px;line-height:1.8;font-size:14px;"><p style="margin:0 0 14px;">您好，${loginAccount}：</p><p style="margin:0 0 14px;">您的原多因素认证（MFA）密钥已失效。下次登录时必须重新绑定新的验证器。</p><div style="margin:22px 0;padding:18px 20px;background:#FFF7ED;border:1px solid #DBEAFE;border-radius:8px;"><p style="margin:0 0 8px;">登录账号：<strong>${loginAccount}</strong></p><p style="margin:0;">重置原因：${reason}</p></div><p style="margin:0;">请前往 <a href="${bindUrl}" style="color:#2563EB;text-decoration:none;">管理系统登录页</a> 重新绑定。</p></div><div style="padding:16px 30px;background:#F3F7FF;color:#64748B;font-size:12px;">Vexra Admin 安全通知，请勿转发。</div></div></div>',
           '{"loginAccount":"admin@example.com","bindUrl":"https://admin.example.com/login","reason":"用户更换设备"}',
           '系统内置模板：管理系统 MFA 重置通知'
    UNION ALL SELECT 'ADMIN_MFA_DISABLED_NOTICE',
           '管理系统 MFA 停用通知',
           '【Vexra Admin】多因素认证（MFA）已停用',
           '<div style="margin:0;padding:36px;background:#F3F7FF;font-family:Arial,''Microsoft YaHei'',sans-serif;color:#0F172A;"><div style="max-width:640px;margin:0 auto;background:#FFFFFF;border:1px solid #DBEAFE;border-radius:8px;overflow:hidden;"><div style="padding:26px 30px;background:#2563EB;color:#FFFFFF;"><div style="font-size:12px;letter-spacing:0;text-transform:uppercase;color:#DBEAFE;">Vexra Admin Security</div><div style="margin-top:8px;font-size:24px;font-weight:700;">多因素认证（MFA）已停用</div></div><div style="padding:30px;line-height:1.8;font-size:14px;"><p style="margin:0 0 14px;">您好，${loginAccount}：</p><p style="margin:0 0 14px;">管理员已停用您账号的多因素认证（MFA）要求。</p><div style="margin:22px 0;padding:18px 20px;background:#F3F7FF;border:1px solid #DBEAFE;border-radius:8px;"><p style="margin:0 0 8px;">登录账号：<strong>${loginAccount}</strong></p><p style="margin:0;">操作原因：${reason}</p></div><p style="margin:0;color:#9a3412;">如果您不清楚该变更来源，请联系系统管理员确认。</p></div><div style="padding:16px 30px;background:#F3F7FF;color:#64748B;font-size:12px;">Vexra Admin 安全通知，请勿转发。</div></div></div>',
           '{"loginAccount":"admin@example.com","reason":"特殊账号调整"}',
           '系统内置模板：管理系统 MFA 停用通知'
    UNION ALL SELECT 'ADMIN_MFA_EXEMPT_NOTICE',
           '管理系统 MFA 豁免通知',
           '【Vexra Admin】多因素认证（MFA）豁免已配置',
           '<div style="margin:0;padding:36px;background:#F3F7FF;font-family:Arial,''Microsoft YaHei'',sans-serif;color:#0F172A;"><div style="max-width:640px;margin:0 auto;background:#FFFFFF;border:1px solid #DBEAFE;border-radius:8px;overflow:hidden;"><div style="padding:26px 30px;background:#2563EB;color:#FFFFFF;"><div style="font-size:12px;letter-spacing:0;text-transform:uppercase;color:#DBEAFE;">Vexra Admin Security</div><div style="margin-top:8px;font-size:24px;font-weight:700;">多因素认证（MFA）豁免</div></div><div style="padding:30px;line-height:1.8;font-size:14px;"><p style="margin:0 0 14px;">您好，${loginAccount}：</p><p style="margin:0 0 14px;">管理员已为您的管理系统账号配置多因素认证（MFA）豁免。</p><div style="margin:22px 0;padding:18px 20px;background:#F3F7FF;border:1px solid #DBEAFE;border-radius:8px;"><p style="margin:0 0 8px;">登录账号：<strong>${loginAccount}</strong></p><p style="margin:0 0 8px;">豁免有效期：${exemptUntil}</p><p style="margin:0;">豁免原因：${reason}</p></div><p style="margin:0;color:#9a3412;">豁免账号仍应使用强密码，并限制共享和转借。</p></div><div style="padding:16px 30px;background:#F3F7FF;color:#64748B;font-size:12px;">Vexra Admin 安全通知，请勿转发。</div></div></div>',
           '{"loginAccount":"admin@example.com","reason":"应急账号","exemptUntil":"长期有效"}',
           '系统内置模板：管理系统 MFA 豁免通知'
) item
WHERE NOT EXISTS (
    SELECT 1

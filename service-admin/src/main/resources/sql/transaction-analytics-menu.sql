...
SELECT app.id, menu.id, 'transaction:analytics:view', '交易分析查询', 'MENU',
       'POST', '/admin/transactions/analytics/**', '查询交易总览和商户表现统计', 1, 0
FROM sys_app app
...
WHERE app.app_code = 'ADMIN'
  ...
  AND NOT EXISTS (
      ...
      WHERE exists_permission.app_id = app.id
        AND exists_permission.permission_code = 'transaction:analytics:view'
...
SET permission.menu_id = menu.id,
    ...
    permission.resource_path = '/admin/transactions/analytics/**',
    permission.description = '查询交易总览和商户表现统计',
    permission.status = 1,
    permission.updated_at = CURRENT_TIMESTAMP(3)
WHERE permission.permission_code = 'transaction:analytics:view'

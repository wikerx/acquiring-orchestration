 ...
 * @author : scott
 * @version : v1.0.0
 * @classname : AdminOperLogServiceImplTests
 * @date : 2026-07-30 18:35
 * @email : scott_x@163.com
 * @description : 验证管理端操作日志只把有业务幂等键的唯一冲突视为 MQ 重复消费
 * @status : create
 */
@Slf4j
class AdminOperLogServiceImplTests {

    @Test
    void shouldTreatIdempotentKeyConflictAsCompletedMqDelivery() {
        log.info("测试管理端日志数据库幂等，关键输入: 非空 idempotentKey、唯一键冲突");
        SysOperLogMapper mapper = duplicateMapper();
        AdminOperLogServiceImpl service = new AdminOperLogServiceImpl(mapper, mock(OperLogConverter.class));
        SysOperLogRecordRequest request = new SysOperLogRecordRequest();
        request.setIdempotentKey("ADMIN-LOG-DB-001");

        assertThatCode(() -> service.recordOperLog(request)).doesNotThrowAnyException();
        log.info("管理端日志数据库幂等测试完成，结果: 唯一冲突按已持久化处理");
    }

    @Test
    void shouldPropagateDuplicateKeyWhenRequestHasNoIdempotentBoundary() {
        log.info("测试管理端日志异常边界，关键输入: idempotentKey 为空、数据库唯一冲突");
        SysOperLogMapper mapper = duplicateMapper();
        AdminOperLogServiceImpl service = new AdminOperLogServiceImpl(mapper, mock(OperLogConverter.class));

        assertThatThrownBy(() -> service.recordOperLog(new SysOperLogRecordRequest()))
                .isInstanceOf(DuplicateKeyException.class);
        log.info("管理端日志异常边界测试完成，结果: 非 MQ 幂等冲突未被吞掉");
    }

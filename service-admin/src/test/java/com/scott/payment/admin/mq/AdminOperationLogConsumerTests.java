 ...
 * @author : scott
 * @version : v1.0.0
 * @classname : AdminOperationLogConsumerTests
 * @date : 2026-07-30 18:30
 * @email : scott_x@163.com
 * @description : 验证管理端操作日志在 Redis 获取、重复和降级三种结果下的数据库幂等与释放边界
 * @status : create
 */
@Slf4j
class AdminOperationLogConsumerTests {

    @Test
    void shouldReleaseIdempotentClaimWhenDatabaseWriteFails() {
        log.info("测试管理端审计失败释放，关键输入: Redis ACQUIRED、数据库不可用");
        AdminOperLogService operLogService = mock(AdminOperLogService.class);
        ...
        OperationLogMessage message = message("ADMIN-LOG-001");
        SysOperLogRecordRequest request = new SysOperLogRecordRequest();
        when(idempotentService.acquireMq("admin-operation-log", "ADMIN-LOG-001",
        ...
        AdminOperationLogConsumer consumer = new AdminOperationLogConsumer(
        ...
        verify(idempotentService).releaseMq(
                "admin-operation-log", "ADMIN-LOG-001", properties.getConsumeIdempotentTtlSeconds());
        log.info("管理端审计失败释放测试完成，结果: 原异常上抛且 Redis 占用已释放");
    }

    @Test
    void shouldContinueToDatabaseWithoutReleaseWhenRedisFallsBack() {
        log.info("测试管理端审计 Redis 降级，关键输入: FALLBACK、数据库写入成功");
        AdminOperLogService operLogService = mock(AdminOperLogService.class);
        IdempotentService idempotentService = mock(IdempotentService.class);
        OperLogMessageConverter converter = mock(OperLogMessageConverter.class);
        OperationLogMqProperties properties = new OperationLogMqProperties();
        OperationLogMessage message = message("ADMIN-LOG-002");
        SysOperLogRecordRequest request = new SysOperLogRecordRequest();
        when(idempotentService.acquireMq("admin-operation-log", "ADMIN-LOG-002",
        ...
        verify(idempotentService, never()).releaseMq(
                "admin-operation-log", "ADMIN-LOG-002", properties.getConsumeIdempotentTtlSeconds());
        log.info("管理端审计 Redis 降级测试完成，结果: 继续数据库唯一约束且未释放 Redis");
    }

    @Test
    void shouldSkipDatabaseForRedisDuplicate() {
        log.info("测试管理端审计重复消息，关键输入: Redis DUPLICATE");
        AdminOperLogService operLogService = mock(AdminOperLogService.class);
        IdempotentService idempotentService = mock(IdempotentService.class);
        OperLogMessageConverter converter = mock(OperLogMessageConverter.class);
        OperationLogMqProperties properties = new OperationLogMqProperties();
        OperationLogMessage message = message("ADMIN-LOG-003");
        when(idempotentService.acquireMq("admin-operation-log", "ADMIN-LOG-003",
        ...
        log.info("管理端审计重复消息测试完成，结果: 未产生数据库写入");
    }

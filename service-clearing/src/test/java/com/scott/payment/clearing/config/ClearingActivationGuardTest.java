 ...
 * @author : scott
 * @version : v1.0.0
 * @classname : ClearingActivationGuardTest
 * @date : 2026-09-02 08:03
 * @email : scott_x@163.com
 * @description : 验证自动清分启动门禁同时保护正式交易拓扑和内部接口 HMAC 边界。
 * @status : create
 */
class ClearingActivationGuardTest {
    ...
    /** 自动清分服务启动时必须提供调用方隔离凭据。 */
    @Test
    void shouldRejectMissingCallerCredentials() {
        InternalServiceAuthProperties authProperties = new InternalServiceAuthProperties();
        ClearingActivationGuard guard = new ClearingActivationGuard(
                new ClearingProperties(), new TransactionShardingProperties(), authProperties);
    ...
    void shouldRejectWhitelistThatBypassesClearingInternalEndpoints() {
        InternalServiceAuthProperties authProperties = authProperties();
    ...
    /** 构造允许 Admin 访问清分内部接口的调用方隔离测试凭据。 */
    private InternalServiceAuthProperties authProperties() {
        InternalServiceAuthProperties.CallerCredential credential =
                new InternalServiceAuthProperties.CallerCredential();
        credential.setActiveSecret("unit-test-clearing-internal-secret-32-bytes");
        credential.setAllowedPaths(List.of("/internal/clearing/**"));

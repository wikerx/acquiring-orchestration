...
对外收单接口使用版本路径控制，例如 `POST /api/rest/payment/v1/authorization`；控制器通过 `@ApiVersion(apiVersion = 1)` 与 `{version}` 路径变量匹配。请求版本高于当前实现版本时，系统自动选择不超过请求版本的最高控制器版本。

系统级非法路径、未知 API、请求方法不支持和参数解析异常由 Web 组件统一拦截，并返回标准 JSON 错误结果。

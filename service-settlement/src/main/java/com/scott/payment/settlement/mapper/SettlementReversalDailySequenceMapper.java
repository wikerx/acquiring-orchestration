 ...
 * @author : scott
 * @version : v1.0.0
 * @classname : SettlementReversalDailySequenceMapper
 * @date : 2026-09-01 00:00
 * @email : scott_x@163.com
 * @description : 冲正申请单号日序列数据访问接口；按 UTC 业务日期锁行并以当前值和版本 CAS 递增。
 * @status : create
 */
public interface SettlementReversalDailySequenceMapper {

    /**
     * 初始化指定 UTC 业务日期的冲正序列行，重复调用不改变现值。
     * @param businessDate UTC 业务日期
     * @return 新插入行数，已存在时为 0
     */
    @Insert("""
            INSERT INTO settlement_reversal_daily_sequence
    ...
    /**
     * 锁定指定日期序列行供当前事务分配下一编号。
     * @param businessDate UTC 业务日期
     * @return 已加行锁的当前序列和版本，不存在时为空
     */
    @Select("""
            SELECT business_date, current_sequence, version
            FROM settlement_reversal_daily_sequence
            WHERE business_date = #{businessDate}
            FOR UPDATE
            """)
    SettlementReversalDailySequenceDO selectForUpdate(@Param("businessDate") LocalDate businessDate);
     ...
     */
    @Update("""
            UPDATE settlement_reversal_daily_sequence

...
CREATE TABLE `payment_checkout_session` (
  ...
  `redirect_url_hash` char(64) DEFAULT NULL COMMENT '结果页返回地址 SHA-256，用于审计排查。',
  `redirect_url_ciphertext` text DEFAULT NULL COMMENT '结果页返回地址 AES-GCM 密文；未上送 redirectUrl 时为空。',
  `redirect_url_encryption_key_version` varchar(64) DEFAULT NULL COMMENT '结果页返回地址密文使用的密钥版本。',
...
CREATE TABLE `transaction_finance_state` (
  ...
  `settlement_rate` decimal(20,10) DEFAULT NULL COMMENT '1 单位交易币种兑换的结算币种数量；至少保留 8 位小数精度。',
...
CREATE TABLE `transaction_order` (
  ...
  `callback_url_ciphertext` text DEFAULT NULL COMMENT '商户通知地址 AES-256-GCM 密文；未上送通知地址时为空。',
  `callback_url_encryption_key_version` varchar(64) DEFAULT NULL COMMENT '商户通知地址密文使用的密钥版本。',
  `callback_url_hash` char(64) DEFAULT NULL COMMENT '商户通知地址 SHA-256，用于审计排查。',
  `redirect_url_ciphertext` text DEFAULT NULL COMMENT 'Hosted Checkout 结果页返回地址 AES-256-GCM 密文；Direct API 或未上送时为空。',
  `redirect_url_encryption_key_version` varchar(64) DEFAULT NULL COMMENT 'Hosted Checkout 结果页返回地址密文使用的密钥版本。',
  `redirect_url_hash` char(64) DEFAULT NULL COMMENT 'Hosted Checkout 结果页返回地址 SHA-256，用于审计排查。',

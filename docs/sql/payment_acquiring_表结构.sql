...
CREATE TABLE `payment_checkout_session` (
  ...
  `allowed_payment_methods_json` json NOT NULL COMMENT '商户本次允许的支付方式和品牌快照，V1 只允许 BANK_CARD + MPGS。',
  ...
  `merchant_return_url` varchar(1024) DEFAULT NULL COMMENT '支付结果页返回商户地址，必须是商户白名单内 HTTPS 地址，禁止携带长期凭证。',
  `merchant_cancel_url` varchar(1024) DEFAULT NULL COMMENT '付款人取消或关闭时返回商户地址，必须是商户白名单内 HTTPS 地址。',
  `merchant_notify_url_hash` char(64) DEFAULT NULL COMMENT '商户异步通知地址 SHA-256，用于审计排查；通知真实地址沿用交易核心配置。',
  `merchant_notify_url_ciphertext` varchar(1024) DEFAULT NULL COMMENT '商户通知地址 AES-GCM 密文',
  `payer_info_ciphertext` text COMMENT '付款人预填信息 AES-GCM 密文',
  `billing_info_ciphertext` text COMMENT '账单预填信息 AES-GCM 密文',
  ...
  `payer_email_masked` varchar(256) DEFAULT NULL COMMENT '付款人邮箱脱敏展示值。',
...
CREATE TABLE `transaction_billing_info` (
  ...
  `transaction_id` varchar(64) NOT NULL COMMENT '平台交易生命周期唯一标识。',
  `operation_id` varchar(64) DEFAULT NULL COMMENT '交易动作ID。',
  `billing_name_masked` varchar(128) DEFAULT NULL COMMENT '账单姓名脱敏展示值。',
  `billing_email_masked` varchar(256) DEFAULT NULL COMMENT '账单邮箱脱敏展示值。',
  `billing_phone_masked` varchar(64) DEFAULT NULL COMMENT '账单手机号脱敏展示值。',
  `billing_country` varchar(3) DEFAULT NULL COMMENT '账单国家/地区。',
  `billing_state` varchar(64) DEFAULT NULL COMMENT '账单州/省。',
  `billing_city` varchar(128) DEFAULT NULL COMMENT '账单城市。',
  `billing_address_line1_masked` varchar(256) DEFAULT NULL COMMENT '账单地址1脱敏值。',
  `billing_address_line2_masked` varchar(256) DEFAULT NULL COMMENT '账单地址2脱敏值。',
...
CREATE TABLE `transaction_merchant_notification` (
  ...
  `notify_config_snapshot_json` json DEFAULT NULL COMMENT '触发时使用的通知配置摘要快照。',
...
CREATE TABLE `transaction_operation` (
  ...
  `channel_request_currency` char(3) DEFAULT NULL COMMENT '上送渠道币种。',
  `channel_request_amount` decimal(20,6) DEFAULT NULL COMMENT '上送渠道金额。',
  `settlement_currency` char(3) DEFAULT NULL COMMENT '动作预计或最终结算币种。',
  `settlement_amount` decimal(20,6) DEFAULT NULL COMMENT '动作预计或最终结算金额。',
...
CREATE TABLE `transaction_order` (
  ...
  `settlement_currency` char(3) DEFAULT NULL COMMENT '预计或最终结算币种。',
  `settlement_amount` decimal(20,6) DEFAULT NULL COMMENT '预计或最终结算金额。',
  ...
  `next_channel_match_time` datetime(3) DEFAULT NULL COMMENT '下一次渠道查询确认计划时间。',
  `channel_match_fail_reason` varchar(512) DEFAULT NULL COMMENT '最近一次渠道查询确认失败原因。',
  `settlement_batch_no` varchar(64) DEFAULT NULL COMMENT '最近一次结算批次号。',
  ...
  `callback_url_hash` char(64) DEFAULT NULL COMMENT '商户通知地址 SHA-256，用于审计排查。',
  `transaction_date_time` datetime(3) NOT NULL COMMENT '交易业务时间，所有交易分表统一字段。',
  `transaction_utc_time` datetime(3) NOT NULL COMMENT '交易业务时间对应 UTC 时间。',
  `transaction_time_zone` varchar(64) NOT NULL DEFAULT 'Asia/Shanghai' COMMENT '交易业务时间所属 IANA 时区。',
...
CREATE TABLE `transaction_payer_info` (
  ...
  `transaction_id` varchar(64) NOT NULL COMMENT '平台交易生命周期唯一标识。',
  `operation_id` varchar(64) DEFAULT NULL COMMENT '交易动作ID。',
  `payer_id` varchar(128) DEFAULT NULL COMMENT '商户侧付款人ID或客户ID。',
  `payer_name_masked` varchar(128) DEFAULT NULL COMMENT '付款人姓名脱敏展示值。',
  `payer_email_masked` varchar(256) DEFAULT NULL COMMENT '付款人邮箱脱敏展示值。',
  `payer_email_hash` char(64) DEFAULT NULL COMMENT '付款人邮箱哈希。',
  `payer_phone_masked` varchar(64) DEFAULT NULL COMMENT '付款人手机号脱敏展示值。',
  `payer_phone_hash` char(64) DEFAULT NULL COMMENT '付款人手机号哈希。',
  `payer_country` varchar(3) DEFAULT NULL COMMENT '付款人国家/地区。',
  `client_ip` varchar(64) DEFAULT NULL COMMENT '付款人客户端IP。',
  `client_ip_country` varchar(3) DEFAULT NULL COMMENT 'IP解析国家/地区。',
  `user_agent_hash` char(64) DEFAULT NULL COMMENT 'User-Agent 哈希。',
  `device_id_hash` char(64) DEFAULT NULL COMMENT '设备ID哈希。',
  `device_info_json` json DEFAULT NULL COMMENT '设备摘要信息。',
  `transaction_date_time` datetime(3) NOT NULL COMMENT '交易业务时间。',
  `transaction_utc_time` datetime(3) NOT NULL COMMENT '交易业务时间对应 UTC 时间。',
  `transaction_time_zone` varchar(64) NOT NULL DEFAULT 'Asia/Shanghai' COMMENT '交易业务时间所属 IANA 时区。',
  `create_time` datetime(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '创建时间。',
...
CREATE TABLE `transaction_payer_info_202603` (
  ...
  KEY `idx_email_hash` (`payer_email_hash`),
  KEY `idx_phone_hash` (`payer_phone_hash`)
) ENGINE=InnoDB AUTO_INCREMENT=202603000000000001 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='保存付款人身份、联系方式、IP、设备摘要，用于风控、审计和后台展示。';
...
CREATE TABLE `transaction_payer_info_202604` (
  ...
  KEY `idx_email_hash` (`payer_email_hash`),
  KEY `idx_phone_hash` (`payer_phone_hash`)
) ENGINE=InnoDB AUTO_INCREMENT=202604000000000001 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='保存付款人身份、联系方式、IP、设备摘要，用于风控、审计和后台展示。';

 ...
 * @description : WorldPay XML 回调处理器，位于 payment-channel-worldpay 渠道实现层，仅负责 WPGXML 基础通知字段解析和状态归一；生产使用前仍必须补齐 WorldPay 签名/认证、IP 白名单和真实通知样例校验。
 * @status : create
 */
public class WorldPayXmlCallbackHandler extends AbstractWorldPayCallbackHandler {
    ...
    public String channelCode() {
        return WorldPayChannelCode.WPGXML;
    }
}

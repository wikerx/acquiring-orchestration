 ...
 * @description : WorldPay JSON HTTP 客户端，位于 payment-channel-worldpay 渠道实现层，负责读取后台 MID 三要素、构造可配置 WPGJSON endpoint、发送 Basic Auth HTTP 请求、记录脱敏请求响应日志，并把渠道响应转换为平台统一渠道结果。
 * @status : create
 */
@Slf4j
public class WorldPayJsonApiClient {
    ...
    /** Worldpay JSON fields containing cardholder or shopper personal data. */
    private static final Pattern WORLDPAY_PERSONAL_FIELD_PATTERN = Pattern.compile(
            "(\"(?:cardHolderName|address1|postalCode|city|state|shopperEmailAddress|emailAddress|email|shopperIp|clientIp|ipAddress|userAgent)\"\\s*:\\s*\")([^\"\\\\]*)(\")",
            Pattern.CASE_INSENSITIVE
    );
    ...
    private void logRequest(ChannelPaymentRequest request,
        ...
        WorldPayPayloadLogMetadata metadata = WorldPayPayloadLogMetadata.from(
                payload == null ? null : JsonUtils.toJsonString(payload));
     ...
     * 对 WPGJSON 请求/响应执行脱敏。
     * <p>
     * 先复用交易交互正文最小脱敏工具，再补充 WorldPay JSON 中卡数据、认证凭据和个人信息字段，
     * 确保测试和审计字段使用同一套规则；应用日志只记录长度和不可逆摘要。
     * </p>
     * @param json 原始 JSON
     * @return 脱敏后的 JSON
     */

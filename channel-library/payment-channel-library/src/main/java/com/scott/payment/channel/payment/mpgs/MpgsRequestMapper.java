 ...
 * @description : MPGS 渠道请求映射组件，位于 payment-channel-library 渠道适配层，负责把平台统一支付请求转换为 MPGS 授权、支付、增量授权、请款、退款和撤销请求字段；输入为 ChannelPaymentRequest，输出为 MpgsRequestPayload，边界限定在本地参数校验、金额币种格式化、卡信息与 3DS 字段组装，不发起渠道网络调用。
 * @status : create
 */
@Component
public class MpgsRequestMapper {
    ...
    /**
     * 构造 MPGS 3DS authentication 节点。
     * <p>
     * dsTransactionId、ECI 和 CAVV 来源于前置 3DS 认证结果；CAVV 属于敏感认证数据，只随渠道请求传递。
     * threeDsVersion 为 3DS1 时补充 threeDs1 状态字段，否则按 3DS2 交易状态字段组装；本方法不重新发起 3DS 认证。
     * </p>
     * @param source 平台统一 3DS 信息，提供目录服务器交易号、ECI、CAVV 和协议版本

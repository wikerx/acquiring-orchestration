 ...
 * @description : WorldPay JSON 请求映射器，位于 payment-channel-library 渠道协议层，负责把平台统一渠道请求转换为 Access Worldpay JSON 请求载荷；输入为 ChannelPaymentRequest，输出为包含 merchant.entity、transactionReference、instruction.value 和 paymentInstrument 的渠道报文，边界限定在本地字段校验、金额最小单位换算和敏感卡字段组装，不发起外部 HTTP 调用。
 * @status : create
 */
@Component

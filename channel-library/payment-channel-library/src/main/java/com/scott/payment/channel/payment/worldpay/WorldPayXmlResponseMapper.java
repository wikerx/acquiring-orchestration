 ...
 * @description : WorldPay XML 响应映射器，位于 payment-channel-library 渠道协议层，负责把 WPGXML 响应对象中的 orderStatus、payment、journal、ok 和 error 信息转换为平台统一渠道响应；输入为 XML 原文或响应对象，输出为 ChannelPaymentResponse。
 * @status : create
 */
@Component
public class WorldPayXmlResponseMapper {
    ...
    public ChannelPaymentResponse toChannelResponse(ChannelPaymentRequest request, WorldPayXmlResponsePayload payload) {
        ...
        response.setChannelCode(PaymentChannelCode.WPGXML.getCode());

 ...
 * @description : 渠道统一交易状态枚举，位于 payment-channel-api 枚举层，用于将渠道原始状态映射为平台可理解的渠道结果，不直接更新平台 transaction_status。
 * @status : create
 */
@Getter
public enum ChannelTradeStatus {
    ...
    /** 渠道已完成授权；是否为平台终态由交易类型决定。 */
    AUTHORIZED("AUTHORIZED"),

    /** 渠道已确认资金捕获，可由平台状态机推进对应交易。 */
    CAPTURED("CAPTURED"),

    /** 渠道已确认退款完成；只允许退款动作据此进入成功终态。 */
    REFUNDED("REFUNDED"),

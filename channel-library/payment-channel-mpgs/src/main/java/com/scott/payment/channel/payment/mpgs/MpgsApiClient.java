 ...
 * @description : MPGS REST API 客户端，位于 payment-channel-mpgs 渠道实现层，负责构造 MPGS URL、Basic Auth、HTTP 调用和请求响应摘要日志；不处理平台交易状态机。
 * @status : create
 */
@Slf4j
public class MpgsApiClient {
    ...
    /**
     * {@code HTTP_METHOD_GET}，表示支付方式、通知方式或调用方式。
     * <p>
    ...
    /**
     * {@code EXT_READ_TIMEOUT_SECONDS}常量，统一 {@code MpgsApiClient} 内部使用的配置值、状态码或协议字段。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * {@code EXT_MPGS_MERCHANT_ID}，用于定位 {@code MpgsApiClient} 关联的上游配置、渠道、账号、角色或业务记录。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /** 平台统一路由结果中的渠道商户号，作为渠道专属 merchantId 元数据的兼容回退。 */
    private static final String EXT_MID_NO = "midNo";

    /**
     * {@code EXT_MPGS_API_USERNAME}，用于展示或识别当前商户、渠道、用户、角色、模板或配置对象。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * {@code EXT_MPGS_PASSWORD}常量，统一 {@code MpgsApiClient} 内部使用的配置值、状态码或协议字段。
     * <p>
    ...
    /**
     * {@code EXT_MPGS_API_PASSWORD}常量，统一 {@code MpgsApiClient} 内部使用的配置值、状态码或协议字段。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；高敏感字段，禁止明文打印日志，禁止写入异常消息。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * {@code EXT_MPGS_API_VERSION}，用于配置快照追踪、缓存代际判断或乐观锁并发控制。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * {@code RAW_RESPONSE_HEADER_JSON_MASKED}，表示 HTTP 请求或响应头集合，敏感头只能记录摘要。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * {@code RAW_RESPONSE_BODY_JSON_MASKED}，表示请求体、响应体或消息载荷，日志中只能保留脱敏摘要。
     * <p>
     * 单位：无；格式：固定协议字面量或受控编码；不允许为空；非敏感字段。
     * 取值范围：取值由当前类对接的协议、状态机或配置约定限定；数据来源：Spring 配置和构造器注入的内部客户端依赖。
     * </p>
    ...
    /**
     * 使用 Retrieve Transaction 查询 MPGS 认证交易的最终状态。
     *
     * @param request 必须包含原订单号和 authentication transaction id
     * @return 当前 3DS 认证状态和认证证据
     */
    public MpgsThreeDsAuthenticationResponse retrieveAuthentication(MpgsThreeDsAuthenticationRequest request) {
        validateProperties(request);
        long startNanos = System.nanoTime();
        String url = null;
        try {
            url = buildAuthenticationUrl(request);
            log.info("MPGS 3DS查询上下文，context: {}", JsonUtils.toJsonString(new ThreeDsRequestLogContext(
                    HTTP_METHOD_GET, MpgsApiOperation.RETRIEVE, request.getOperationId(), request.getTransactionId(),
                    request.getChannelOrderNo(), request.getAuthenticationTransactionId(), request.getMerchantOrderNo()
            )));
            HttpResponse<String> response = sendGet(request, url);
            return handleAuthenticationResponse(
                    request, response, HTTP_METHOD_GET, MpgsApiOperation.RETRIEVE, url, startNanos);
        } catch (java.net.http.HttpTimeoutException e) {
            logThreeDsException(request, MpgsApiOperation.RETRIEVE, url, startNanos, e);
            throw new ChannelTimeoutException("MPGS 3DS retrieve timed out", e);
        } catch (IOException e) {
            logThreeDsException(request, MpgsApiOperation.RETRIEVE, url, startNanos, e);
            throw new ChannelRequestException("MPGS 3DS retrieve network request failed", e, true);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            logThreeDsException(request, MpgsApiOperation.RETRIEVE, url, startNanos, e);
            throw new ChannelRequestException("MPGS 3DS retrieve was interrupted", e, true);
        }
    }
    ...
    private MpgsThreeDsAuthenticationResponse executeAuthentication(MpgsThreeDsAuthenticationRequest request,
        ...
        try {
            ...
            return handleAuthenticationResponse(
                    request, response, HTTP_METHOD_PUT, apiOperation, url, startNanos);
        } catch (java.net.http.HttpTimeoutException e) {
    ...
    private ChannelPaymentResponse handleResponse(ChannelPaymentRequest request,
        ...
        if (!StringUtils.hasText(body)) {
            throw responseException("MPGS response body is empty", response.statusCode(), null);
        }
        ...
        if ((response.statusCode() < 200 || response.statusCode() >= 300) && !hasMpgsResult(payload)) {
            throw responseException(
                    "MPGS HTTP response is not successful, status: " + response.statusCode(),
    ...
    private MpgsResponsePayload parseResponseBody(String body, int httpStatus) {
        try {
            ...
            if (httpStatus < 200 || httpStatus >= 300) {
                throw responseException(
                        "MPGS HTTP response is not successful, status: " + httpStatus, httpStatus, e);
            }
            throw new ChannelResponseException("MPGS response parse failed", e);
        }
    }

    /** 明确的客户端请求拒绝可直接失败；可能发生在处理途中的冲突、限流和超时响应仍需后续勾兑。 */
    private ChannelResponseException responseException(String message, int httpStatus, Throwable cause) {
        boolean outcomeUncertain = httpStatus < 400
                || httpStatus >= 500
                || httpStatus == 408
                || httpStatus == 409
                || httpStatus == 425
                || httpStatus == 429;
        return new ChannelResponseException(message, cause, outcomeUncertain);
    }
    ...
    /** 使用 GET 查询 MPGS 3DS authentication transaction。 */
    private HttpResponse<String> sendGet(MpgsThreeDsAuthenticationRequest request, String url)
    ...
    String buildTransactionUrl(ChannelPaymentRequest request) {
        ...
        return baseUrl
                ...
                + "/merchant/" + encode(mpgsMerchantId(request))
    ...
    private String basicAuthHeader(ChannelPaymentRequest request) {
        String merchantId = mpgsMerchantId(request);
    ...
    private void validateProperties(ChannelPaymentRequest request) {
        ...
        requireText(mpgsMerchantId(request), "MPGS merchantId is required");
    ...
    private MpgsThreeDsAuthenticationResponse handleAuthenticationResponse(MpgsThreeDsAuthenticationRequest request,
        ...
        log.info("MPGS 3DS响应上下文，context: {}", JsonUtils.toJsonString(new ThreeDsResponseLogContext(
                httpMethod, apiOperation, request.getOperationId(), request.getTransactionId(),
        ...
        if (!StringUtils.hasText(body)) {
            throw responseException("MPGS 3DS response body is empty", response.statusCode(), null);
        }
        MpgsResponsePayload payload = parseResponseBody(body, response.statusCode());
        if ((response.statusCode() < 200 || response.statusCode() >= 300) && !hasMpgsResult(payload)) {
            throw responseException(
                    "MPGS 3DS HTTP response is not successful, status: " + response.statusCode(),
                    response.statusCode(), null);
        }
        ...
        result.getExtension().put("httpMethod", httpMethod);
    ...
    private MpgsThreeDsAuthenticationResponse toThreeDsAuthenticationResponse(MpgsThreeDsAuthenticationRequest request,
        ...
        MpgsResponsePayload.Transaction transaction = payload == null ? null : payload.getTransaction();
        MpgsResponsePayload.Order order = payload == null ? null : payload.getOrder();
        ...
        response.setAuthenticationStatus(firstText(
                authentication == null ? null : authentication.getStatus(),
                transaction == null ? null : transaction.getAuthenticationStatus(),
                order == null ? null : order.getAuthenticationStatus()));
    ...
    /** 按渠道专属元数据、平台统一 MID、渠道本地配置的顺序解析交易 merchantId。 */
    private String mpgsMerchantId(ChannelPaymentRequest request) {
        return firstText(
                extensionValue(request, EXT_MPGS_MERCHANT_ID, null),
                extensionValue(request, EXT_MID_NO, null),
                properties.getMerchantId());
    }

    /** 按渠道专属元数据、平台统一 MID、渠道本地配置的顺序解析 3DS merchantId。 */
    private String mpgsMerchantId(MpgsThreeDsAuthenticationRequest request) {
        return firstText(
                extensionValue(request, EXT_MPGS_MERCHANT_ID, null),
                extensionValue(request, EXT_MID_NO, null),
                properties.getMerchantId());
